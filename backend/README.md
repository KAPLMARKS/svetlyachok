...
| `POST` | `/api/v1/auth/logout` | Серверный logout (аудит; клиент стирает токены) |
...
| `POST` | `/api/v1/fingerprints/batch` | Bulk-приём live-отпечатков (до 100 за запрос); partial success |
| `GET` | `/api/v1/fingerprints` | Список отпечатков с фильтрами (admin) |
| `GET` | `/api/v1/fingerprints/{id}` | Детали отпечатка (admin) |
| `POST` | `/api/v1/calibration/points` | Создать калибровочную точку (admin); инвалидирует ML-кэш |
| `GET` | `/api/v1/calibration/points` | Список калибровочных точек (любой авторизованный) |
| `DELETE` | `/api/v1/calibration/points/{id}` | Удалить калибровочную точку (admin); 400 если это live-отпечаток; инвалидирует ML-кэш |
...
curl -X POST http://localhost:8000/api/v1/auth/login \
  ...
  -d '{"email":"admin@svetlyachok.dev","password":"admin12345"}'
```
...
Ответ — `200 {"message": "logged_out"}`. Сервер логирует
`employee_id` для аудита, но **не делает blacklist'а access-токена**:
до истечения срока (30 минут) токен формально остаётся валидным.
Клиент обязан стереть свои `access` и `refresh` локально. Полноценный
revocation с таблицей `revoked_tokens` отложен в security hardening.

Rate limit — `10/minute` (env `AUTH_LOGOUT_RATE_LIMIT`), защита от
ddos через бесконечные logout-вызовы.

### Rate limiting

`/auth/login` ограничен **5 запросами в минуту на IP**, `/auth/refresh`
и `/auth/logout` — **по 10 в минуту**. Превышение → `429 Too Many
Requests` + заголовок `Retry-After`. Лимиты конфигурируются
переменными окружения `AUTH_LOGIN_RATE_LIMIT`,
`AUTH_REFRESH_RATE_LIMIT` и `AUTH_LOGOUT_RATE_LIMIT` (формат slowapi:
`<count>/<period>`).
...
### Bulk-приём отпечатков (mobile offline-кэш)

Mobile-приложение копит отпечатки в локальном кэше при отсутствии сети
и шлёт пачкой при восстановлении. Чтобы не дёргать N HTTP-запросов
(батарея, трафик), используется `POST /api/v1/fingerprints/batch`:

```bash
curl -X POST http://localhost:8000/api/v1/fingerprints/batch \
  -H "Authorization: Bearer ${ACCESS}" \
  -H "Content-Type: application/json" \
  -d '{
    "items": [
      {"captured_at": "2026-05-01T12:34:56Z", "rssi_vector": {"AA:BB:CC:DD:EE:01": -45}, "sample_count": 1},
      {"captured_at": "2026-05-01T12:35:30Z", "rssi_vector": {"AA:BB:CC:DD:EE:01": -47}, "sample_count": 1}
    ]
  }'
```

- Лимит: до **100 items** за запрос (Pydantic-validation, превышение → `422`).
- Семантика — **partial success**: один невалидный item не валит весь
  batch. Каждый item обрабатывается тем же `SubmitFingerprintUseCase`,
  что и одиночный `POST /fingerprints`.
- Ответ — `200 OK` со структурой:

```json
{
  "accepted": [{"index": 0, "fingerprint": { /* FingerprintResponse */ }}],
  "rejected": [{"index": 1, "code": "captured_at_in_future", "message": "..."}],
  "accepted_count": 1,
  "rejected_count": 1
}
```

`index` соответствует позиции item'а в исходном `items[]` —
mobile-клиент удаляет из локального кэша записи по `accepted[].index`,
а по `rejected[].code` решает: удалить как нерешаемую ошибку
(`captured_at_too_old`) или оставить для retry.
...
Модель инвалидируется автоматически после CRUD на калибровочных
точках: `POST /api/v1/calibration/points` и
`DELETE /api/v1/calibration/points/{id}` после успешного выполнения
вызывают `invalidate_position_classifier_cache()` в presentation-слое
(см. `app/presentation/dependencies.py`). На следующий `/classify`
создаётся новый instance классификатора и обучается на актуальной
калибровочной выборке. Дополнительно рестарт backend'а не нужен.
