...
class FingerprintRepository(Protocol):
    ...
    async def list_calibrated_all(self) -> List[Fingerprint]:
        """Все калибровочные отпечатки во всех зонах.

        Используется ML-классификатором для тренировки на полном
        наборе. Без пагинации: на пилоте десятки-сотни точек,
        весь список умещается в памяти. Для production-нагрузки
        потребуется streaming/batch-обучение — отдельная задача.
        """
        ...
